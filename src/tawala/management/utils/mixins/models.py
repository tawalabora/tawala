...
class OrderedChoiceMixin:
    ...
    choice_field: ClassVar[str] = "name"
    order_field: ClassVar[str] = "order"

    def save(self, *args: Any, **kwargs: Any) -> None:
        choice_value: Optional[str] = getattr(self, self.choice_field, None)
        order_attr: Optional[int] = getattr(self, self.order_field, None)

        if choice_value is not None and order_attr is not None:
            field: Field = self._meta.get_field(self.choice_field)  # type: ignore[attr-defined]
            choices = cast(Optional[List[Tuple[Any, str]]], field.choices)  # type: ignore[misc]
            if choices:
                choices_values: List[Any] = [choice[0] for choice in choices]
                if choice_value in choices_values:
                    # Set order based on position in choices (1-indexed)
                    setattr(self, self.order_field, choices_values.index(choice_value) + 1)
                else:
...
class CategoryAssignmentMixin:
    ...
    choice_field: ClassVar[str] = "name"
    category_field: ClassVar[str] = "category"
    category_mapping: ClassVar[Dict[Any, Any]] = {}  # Must be overridden in subclass
    ...
    def save(self, *args: Any, **kwargs: Any) -> None:
        ...
        """
        choice_value: Optional[Any] = getattr(self, self.choice_field, None)

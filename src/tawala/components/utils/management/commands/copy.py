...
class FileCopier(Copier):
    ...
    def copy(self, source: Path, destination: Path) -> bool:
        ...
        if not source.is_file():
            self.command.stdout.write(
                self.command.style.ERROR(f"Source is not a file: {source}")
            )
            return False
        ...
        try:
            ...
            self.command.stdout.write(
                self.command.style.ERROR(
                    f"Failed to copy file: {type(e).__name__}: {e}"
                )
            )
            return False
...
class DirectoryCopier(Copier):
    ...
    def _prompt_overwrite(self, destination: Path) -> bool:
        ...
        """
        prompt: str = (
            f"\n{self.command.style.WARNING(str(destination))} already exists. "
            f"Overwrite? [y/N]: "
        )
